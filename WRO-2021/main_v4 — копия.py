...
def on_message(client, userdata, message):
    global bot_ready
    global carnum
    if message.topic == 'techbkrill/from_bot_to_cam':
...
while key != 8:
    ...
    lower_blue = np.array([119, 95, 89])
    upper_blue = np.array([142, 255, 255])

    lower_green = np.array([65, 89, 113])
    upper_green = np.array([107, 237, 243])

    lower_red = np.array([178, 130, 0])
    upper_red = np.array([255, 255, 255])

    lower_violet = np.array([184, 36, 125])  # 160, 26, 83
    upper_violet = np.array([225, 119, 184])  # 255, 255, 255

    lower_yellow = np.array([24, 119, 130])
    ...
    res_violet = cv2.bitwise_and(image, image, mask=violet_mask)
    ...
    for contour_rd in contours_red:
        ...
        if area_rd > 1750:
            cv2.circle(image, (w_rd // 2 + x_rd, h_rd // 2 + y_rd), 7, (0, 255, 0), -1)
            cv2.drawContours(image, [box_rd], 0, (255, 255, 255), 4)
        if points_rd > 50:
    ...
    for contour_gr in contours_green:
        ...
        if area_gr > 1750:
            cv2.circle(image, (w_gr // 2 + x_gr, h_gr // 2 + y_gr), 7, (0, 255, 0), -1)
            cv2.drawContours(image, [box_gr], 0, (255, 255, 255), 4)
    ...
    for contour_bl in contours_blue:
        ...
        if area_bl > 1750:
            cv2.circle(image, (w_bl // 2 + x_bl, h_bl // 2 + y_bl), 7, (0, 255, 0), -1)
            cv2.drawContours(image, [box_bl], 0, (255, 255, 255), 4)
    ...
    for contour_vl in contours_violet:
        ...
        if area_vl > 1750:
            cv2.circle(image, (w_vl // 2 + x_vl, h_vl // 2 + y_vl), 7, (0, 255, 0), -1)
            cv2.drawContours(image, [box_vl], 0, (255, 255, 255), 4)
    ...
    for contour_yl in contours_yellow:
        ...
        if area_yl > 1750:
            cv2.circle(image, (w_yl // 2 + x_yl, h_yl // 2 + y_yl), 7, (0, 255, 0), -1)
            cv2.drawContours(image, [box_yl], 0, (255, 255, 255), 4)
    ...
    cv2.line(image, (green_x, green_y), (yellow_x, yellow_y), (200, 0, 0), 2)
    cv2.line(image, (yellow_x, yellow_y),(blue_x, blue_y), (200, 0, 0), 2)
    cv2.line(image, (violet_y, violet_y), (red_x, red_y), (200, 0, 0), 2)
    ...
    cv2.putText(image, str(distance_yl_to_gr), (100,40), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 1)
